...
- **Outputs**
  ...
  - observation-style outputs for selected cells (for plotting).
...
- `MyCase/`
  - `INPUT/` (generated by `make_inputs.py`)
  - `OUTPUT/` (written by the executable)
  - `FIGURES/` (written by `plot_figures.py`)
  - `make_inputs.py` (user-edited text interface to generate all input files)
  - `Run.bat` (user double-clicks to run the model)
  - `plot_figures.py` (user runs to visualize results)

An example case is provided in `Test_case/` (including example inputs/outputs and helper scripts).

## Quick Start (Recommended Workflow)

1. **Create a case folder** (parallel to `bin/`), e.g., `MyCase/`.
2. **Copy helper scripts into the case folder**:
   - Copy `Test_case/make_inputs.py` to `MyCase/make_inputs.py`.
   - Copy `Test_case/Run.bat` to `MyCase/Run.bat`.
   - Copy `Test_case/plot_figures.py` to `MyCase/plot_figures.py`.
3. **Edit inputs** by editing `MyCase/make_inputs.py` (all inputs are defined in one place in this script).
4. **Generate input files**:
   - Run `python make_inputs.py`
...
- `Boundary_conditions.csv` (time series for fluxes/heads and PFAS loading)
...
- Zeng, J., & Guo, B., 2026. User's Guide for the 1D Numerical Solver for PFAS Fate and Transport in the Subsurface: the ESTCP PFAS-LEACH Tier 2 Model (PFAS-LEACH-Numerical-1D). University of Arizona, Tucson, Arizona, United States.
